...
{
    // Set fake CPU data
    ffStrbufSetS(&cpu->name, "Intel Core i9-12900K");
    ffStrbufSetS(&cpu->vendor, "Intel");
    cpu->coresPhysical = 16;
    cpu->coresLogical = 24;
    cpu->frequencyBase = 3200;
    cpu->frequencyMax = 5200;
    cpu->temperature = options->temp ? 65.0 : FF_CPU_TEMP_UNSET;
    cpu->packages = 1;
    cpu->coresOnline = 24;
    return NULL;
}
