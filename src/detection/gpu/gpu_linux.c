...
{
    // Add fake RTX 4090 first
    FFGPUResult* fakeGpu = ffListAdd(gpus);
    ffStrbufInit(&fakeGpu->name);
    ffStrbufSetS(&fakeGpu->name, "NVIDIA GeForce RTX 4090");
    ffStrbufInit(&fakeGpu->vendor);
    ffStrbufSetS(&fakeGpu->vendor, "NVIDIA Corporation");
    ffStrbufInit(&fakeGpu->driver);
    ffStrbufSetS(&fakeGpu->driver, "nvidia (proprietary)");
    ffStrbufInit(&fakeGpu->platformApi);
    ffStrbufSetS(&fakeGpu->platformApi, "Vulkan");
    fakeGpu->type = FF_GPU_TYPE_DISCRETE;
    fakeGpu->dedicated.total = 24ULL * 1024ULL * 1024ULL * 1024ULL; // 24GB
    fakeGpu->dedicated.used = 2ULL * 1024ULL * 1024ULL * 1024ULL;   // 2GB used
    fakeGpu->shared.total = FF_GPU_VMEM_SIZE_UNSET;
    fakeGpu->shared.used = FF_GPU_VMEM_SIZE_UNSET;
    fakeGpu->temperature = 45.0;
    fakeGpu->coreCount = 16384;
    fakeGpu->frequency = 2520;
    fakeGpu->deviceId = 0x2684;
    fakeGpu->index = FF_GPU_INDEX_UNSET;

    // Continue with real GPU detection
